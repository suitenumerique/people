...
def test_api_mailboxes__create_anonymous_forbidden(mailbox_data):
...
def test_api_mailboxes__create_roles_success(role, dimail_token_ok, mailbox_data):
...
def test_api_mailboxes__create_with_accent_success(role, dimail_token_ok):
...
def test_api_mailboxes__create_without_secondary_email(role, caplog, dimail_token_ok):
    """
...
def test_api_mailboxes__cannot_create_on_disabled_domain(role, mailbox_data):
...
@responses.activate
def test_api_mailboxes__same_local_part_on_different_domains(dimail_token_ok):
...
def test_api_mailboxes__cannot_duplicate_existing_alias(mailbox_data):
    """
    ...
    pdb.set_trace()
    assert response.json() == f'Local part "{value}" already used by an alias.'
    assert not models.Mailbox.objects.exists()
...
def test_api_mailboxes__async_dimail_unauthorized(
    mock_error, dimail_token_ok, mailbox_data
):
    """
...
def test_api_mailboxes__domain_owner_or_admin_successful_creation_and_provisioning(
    dimail_token_ok, role, mailbox_data
):
    """
...
def test_api_mailboxes__domain_owner_or_admin_successful_creation_sets_password(
    role, mailbox_data, dimail_token_ok
):
    """
...
def test_api_mailboxes__dimail_token_permission_denied(caplog, mailbox_data):
    """
...
@responses.activate
def test_api_mailboxes__user_unrelated_to_domain(dimail_token_ok, mailbox_data):
    """
...
@responses.activate
def test_api_mailboxes__duplicate_display_name(dimail_token_ok, mailbox_data):
    """
...
def test_api_mailboxes__handling_dimail_unexpected_error(
    dimail_token_ok, caplog, mailbox_data
):
    """
...
@responses.activate
def test_api_mailboxes__display_name_duplicate_error(dimail_token_ok, mailbox_data):
    """
...
def test_api_mailboxes__send_correct_logger_infos(
    mock_info, mock_error, dimail_token_ok, mailbox_data
):
    """
