...
class DomainResource(ModelResource):
    ...
    class Meta:
        ...
        fields = ["name", "slug", "status", "support_email", "owners", "admins"]

    def get_queryset(self):
        """Prefetch related accesses' users for performance."""
        return super().get_queryset().prefetch_related("accesses__user")

    def dehydrate_owners(self, domain):
        """Get domain's owners list."""
        return [
            access.user.email
            for access in domain.accesses.all()
            if access.role == enums.MailDomainRoleChoices.OWNER
        ]

    def dehydrate_admins(self, domain):
        """Get domain's admins list."""
        return [
            access.user.email
            for access in domain.accesses.all()
            if access.role == enums.MailDomainRoleChoices.ADMIN
        ]
...
class MailDomainAdmin(ExportActionMixin, admin.ModelAdmin):
    ...
    def sync_aliases_from_dimail(self, request, queryset):  # pylint: disable=unused-argument
        """
    ...
    def fetch_domain_status_from_dimail(self, request, queryset):  # pylint: disable=unused-argument
        ...
        if success:
            msg_success = [
                ...
                _("Domains updated: %(domains)s")
                % {"domains": ", ".join(domains_updated)}
                if domains_updated
            ...
            messages.success(
                request,
                format_html_join(
                    mark_safe("<br> "), "{}", ([str(m)] for m in msg_success)
                ),
            )
        if msg_error:
            ...
            messages.error(
                request,
                format_html_join(
                    mark_safe("<br> "), "{}", ([str(m)] for m in msg_error)
                ),
            )
        if excluded_domains:
            messages.warning(
    ...
    def fetch_domain_expected_config_from_dimail(self, request, queryset):  # pylint: disable=unused-argument
    ...
    def send_pending_mailboxes(self, request, queryset):  # pylint: disable=unused-argument
...
class MailboxAdmin(admin.ModelAdmin):
    ...
    list_display = ("__str__", "domain", "status", "updated_at", "password")
    list_filter = ("status",)
