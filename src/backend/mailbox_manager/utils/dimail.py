...
class DimailAPIClient:
    ...
    def get_domain(self, domain_name):
        """Send a GET domain request to dimail API."""
        try:
            response = session.get(
                f"{self.API_URL}/domains/{domain_name}/",
    ...
    def fetch_domain_status(self, domain):
        ...
        return self.compute_domain_status(domain, dimail_response)

    def compute_domain_status(self, domain, check_response):
        dimail_state = check_response["state"]
        ...
        if (
            ...
            domain.status = enums.MailDomainStatusChoices.ENABLED
            domain.last_check_details = check_response
            domain.save()
        ...
        # and manage the case where the domain has to be fixed by support
            ...
            external_checks = self._get_dimail_checks(check_response, internal=False)
            internal_checks = self._get_dimail_checks(check_response, internal=True)
