...
def test_matrix_webhook__invite_user_to_room_forbidden(caplog):
    """Cannot invite when People's service account is not in the room."""
    ...
    with responses.RequestsMock() as rsps:
        ...
        rsps.add(
            rsps.POST,
            re.compile(r".*/invite"),
            body=str(matrix.mock_request_successful),
            status=status.HTTP_200_OK,
            content_type="application/json",
        )
...
def test_matrix_webhook__invite_user_to_room_success(caplog):
    ...
    with responses.RequestsMock() as rsps:
        ...
        rsps.add(
            rsps.POST,
            re.compile(r".*/join"),
            body=str(matrix.mock_ok_room_joined.message),
            status=matrix.mock_ok_room_joined.status_code,
            content_type="application/json",
        )
        rsps.add(
            rsps.POST,
            re.compile(r".*/invite"),
            body=str(matrix.mock_request_successful.message),
            status=matrix.mock_request_successful.status_code,
            content_type="application/json",
        )
        webhook_synchronizer.invite_user_to_room(team=webhook.team, user=user)
