"""Authentication for the la suite plugin."""
...
class OrganizationOneTimeTokenAuthentication(TokenAuthentication):
    ...
    def authenticate_credentials(
        self, key
    ) -> Tuple[models.Organization, models.OrganizationOneTimeToken]:
        """
        ...
        if not self.organization_can_authenticate(token.organization):
            logger.warning(
                "Token cannot authenticate organization: %s", token.organization
            )
            raise exceptions.AuthenticationFailed(
                _("Token cannot be used for authentication.")
            )
